<template>
    <el-dialog title="入库" :visible.sync="dialogVisible" :before-close="handleClose" width="650px">
        <div>确认将以下订单入库？</div>
...
export default {
    ...
    methods: {
        ...
        async handleSubmit () {
            try {
                this.loading = true
                await httpPost(`/api/admin/orders/storage`, { orderIdList: this.orderIdList })
                this.$alert('操作成功')
                this.handleClose()
