<template>
  <div class="g-core-image-upload-btn">
    <slot>{{text}}</slot>
    <div class="g-core-image-upload-form" v-show="!hasImage">
      <input v-bind:disabled="uploading" ref="input" v-bind:name="name" v-bind:multiple="multiple" type="file" v-bind:accept="inputAccept" v-on:change="change">
    </div>
     <div class="g-core-image-corp-container" ref="modal" v-show="hasImage">
      ...
      <div class="info-aside">
        ...
        <p class="btn-groups" v-if="crop">
          <el-button type="primary" v-on:click="doCrop" :loading="uploading">确认上传</el-button>
...
export default {
    ...
    watch: {
        hasImage (newValue) {
            this.$emit('has-image-change', newValue)
        }
    },
    ...
    methods: {
        ...
        __find (str) {
            return this.$refs.modal.querySelector(str)
        },
        change (e) {
            let fileVal = this.$refs.input.value.replace(/C:\\fakepath\\/i, '')
